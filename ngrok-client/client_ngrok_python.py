...
MAIN_HOST = config.MAIN_HOST  # Ngrok服务器地址
MAIN_PROT = config.MAIN_PROT  # 端口
BUF_SIZE = 1024  # 吞吐量
Tunnels = config.TUNNEL_LIST  # 全局渠道赋值
MAIN_SOCKET = 0
CLIENT_ID = ""
PING_TIME = 0
        ...
        try:
            if linkstate == 0:
                if type == 1:
                    ...
                    linkstate = 1
                if type == 2:
                    sendpack(sock, RegProxy(CLIENT_ID), False)
                    linkstate = 1
                if type == 3:
                    linkstate = 1

            recvbut = sock.recv(BUF_SIZE)
            ...
            if type == 1 or (type == 2 and linkstate == 1):
                ...
                if len(recvbuf) >= (8 + lenbyte):
                    ...
                    if type == 1:
                        if js['Type'] == 'ReqProxy':
                            newsock = connectremote(MAIN_HOST, MAIN_PROT)
                            if newsock:
                        ...
                        if js['Type'] == 'AuthResp':
                            ...
                            logger.info('Authenticated with server, client id: %s' % CLIENT_ID)
                            sendpack(sock, Ping())
                            PING_TIME = time.time()
                            for tunnelinfo in Tunnels:
                                # 注册通道
...
if __name__ == '__main__':
    ...
    while True:
        try:
            # 检测控制连接是否连接.
            if not MAIN_SOCKET:
                ip = dnsopen(MAIN_HOST)
                if ip == False:
                    logger = logging.getLogger('%s' % 'client')
                    logger.info('update dns')
                    time.sleep(10)
                    continue
                MAIN_SOCKET = connectremote(ip, MAIN_PROT)
                if not MAIN_SOCKET:
                    logger = logging.getLogger('%s' % 'client')
                    logger.info('connect failed...!')
                    time.sleep(10)
                    continue
                thread = threading.Thread(target = HKClient, args = (MAIN_SOCKET, 0, 1))
                thread.setDaemon(True)
                thread.start()

            # 发送心跳
            if PING_TIME + 20 < time.time() and PING_TIME != 0:
                sendpack(MAIN_SOCKET, Ping())
                PING_TIME = time.time()
